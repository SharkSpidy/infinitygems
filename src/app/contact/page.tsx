      ...
      <div className="max-w-6xl mx-auto px-6">
        ...
        <div className="text-center mb-20 max-w-xl mx-auto">
          ...
          <p className="font-sans text-sm text-silver font-light leading-relaxed">
            For general inquiries, provenance questions, or to discuss acquiring a
            specific specimen from our vault.
          </p>
        </div>
          ...
          <div className="lg:col-span-2 space-y-10">
            {[
              {
                icon: (
                  <svg width="18" height="18" viewBox="0 0 18 18" fill="none">
                    <path d="M9 1C5.69 1 3 3.69 3 7c0 4.5 6 10 6 10s6-5.5 6-10c0-3.31-2.69-6-6-6zm0 8.5A2.5 2.5 0 119 4a2.5 2.5 0 010 5.5z" stroke="#C9A84C" strokeWidth="0.8" fill="none"/>
                  </svg>
                ),
                title: 'The Vault',
                lines: ['12 Gemstone Quarter', 'Suite 400, The Meridian Tower', 'Geneva, Switzerland — 1201'],
              },
              {
                icon: (
                  <svg width="18" height="18" viewBox="0 0 18 18" fill="none">
                    <path d="M3 4h12v10H3V4z" stroke="#C9A84C" strokeWidth="0.8"/>
                    <path d="M3 4l6 6 6-6" stroke="#C9A84C" strokeWidth="0.8"/>
                  </svg>
                ),
                title: 'Electronic',
                lines: ['vault@infinitygems.com', 'acquisitions@infinitygems.com'],
              },
              {
                icon: (
                  <svg width="18" height="18" viewBox="0 0 18 18" fill="none">
                    <path d="M4 2h4l1.5 4L8 7.5s1 2 2.5 2.5L12 8l4 1.5v3.5C16 15 14 16 12 16 7 16 2 11 2 6c0-2 1-4 2-4z" stroke="#C9A84C" strokeWidth="0.8" fill="none"/>
                  </svg>
                ),
                title: 'Telephone',
                lines: ['+41 22 000 0000', 'Mon–Fri, 09:00–18:00 CET'],
              },
            ].map(({ icon, title, lines }) => (
              <div key={title} className="flex gap-5">
                <div className="w-8 h-8 border border-iron/40 flex items-center justify-center flex-shrink-0 mt-0.5">
                  {icon}
                </div>
                <div>
                  <h3 className="font-sans text-xs tracking-widest uppercase text-gold mb-2">{title}</h3>
                  {lines.map((l) => (
                    <p key={l} className="font-sans text-sm text-silver font-light">{l}</p>
                  ))}
                </div>
              </div>
            ))}
            ...
            <div className="border border-iron/20 p-5 bg-charcoal/20">
              <p className="font-sans text-xs text-silver/50 leading-relaxed font-light">
                All communications are treated with the utmost discretion. We do not
                disclose client information to third parties under any circumstances.
              </p>
            </div>
          </div>
